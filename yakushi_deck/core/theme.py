...
def _rofi_concrete_override(
    ...
    # Theme Studio owns RGB values; Rofi Studio owns the shared alpha.
    # Every background surface receives the same alpha, while text/icons remain
    # fully opaque.
    ...
    surface_rgba = _rgba_rasi(surface, opacity, DEFAULTS["surface"])
    surface_alt_rgba = _rgba_rasi(surface_alt, opacity, DEFAULTS["surface_alt"])
    hover_rgba = _rgba_rasi(hover, opacity, DEFAULTS["hover_bg"])
...
def save(palette: Palette) -> tuple[bool, str]:
    ...
    rofi_override = _rofi_concrete_override(values["bg"], rofi_opacity, values["surface"], values["surface_alt"], values["hover_bg"])
