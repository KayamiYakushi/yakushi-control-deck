...
def _override_alpha_fraction(text: str) -> float | None:
    # Raycast Glass stores the user-facing master opacity on mainbox while its
    # outer window is only a much lighter faux shadow. Prefer the panel value
    # whenever it exists so Theme Studio never mistakes shadow alpha for the
    # launcher's configured opacity.
    mainbox = re.search(
        r'(?ms)^\s*mainbox\s*\{.*?^\s*background-color\s*:\s*rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*([0-9.]+)%?\s*\)',
        text or "",
        flags=re.I,
    )
    if mainbox:
        raw = float(mainbox.group(1))
...
def _rofi_concrete_override(
    ...
    shadow_rgba = _rgba_rasi("#000000", opacity * 0.18, "#000000")
...
def save(palette: Palette) -> tuple[bool, str]:
    ...
    shadow_rofi = premium_rofi and "/* YAKUSHI ROFI SHADOW: inset */" in rofi_original
