...
backup_file "$HOME/.config/rofi/config.rasi" rofi/config.rasi; backup_file "$HOME/.config/rofi/scripts" rofi/scripts
...
cp -a "$ROOT/yakushi_deck" "$TARGET/"; cp -a "$ROOT/README.md" "$ROOT/LICENSE" "$TARGET/"
...
cp -a "$ROOT/integrations/rofi/config.rasi" "$HOME/.config/rofi/config.rasi"; cp -a "$ROOT/integrations/rofi/scripts/." "$HOME/.config/rofi/scripts/"
...
printf '{"version":"1.0.0","backup":"%s","installed_at":"%s"}\n' "$BACKUP" "$STAMP" > "$DATA/install.json"
say ''; say '薬  Yakushi Control Deck 1.0.0 installed.'; say "Backup: $BACKUP"; say 'Left click 薬 -> Control Deck'; say 'Right click 薬 -> Rofi'; say 'Power button -> Rofi power menu'; say 'Run ./doctor.sh for diagnostics.'
